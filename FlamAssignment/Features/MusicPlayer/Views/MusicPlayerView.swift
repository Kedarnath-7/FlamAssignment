    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                Color.dynamicBackground(for: musicManager.currentSong)
                    .ignoresSafeArea()
                    .animation(.easeInOut(duration: 1.0), value: musicManager.currentSong?.id)
            ...
            .preferredColorScheme(.dark) // Force dark mode for better contrast with dynamic backgrounds
        ...
        VStack(spacing: 24) {
            ...
            HStack(spacing: 60) {
                ...
                Button(action: musicManager.togglePlaybackMode) {
                    VStack(spacing: 6) {
                        ZStack {
                            Circle()
                                .fill(musicManager.playbackMode == .normal ? Color(.systemGray6) : Color.dynamicAccent(for: musicManager.currentSong).opacity(0.2))
                                .frame(width: 44, height: 44)
                                .shadow(color: Color.black.opacity(0.08), radius: 2, x: 0, y: 1)
                            
                            Image(systemName: musicManager.playbackMode.iconName)
                                .font(.system(size: 16, weight: .semibold))
                                .foregroundColor(musicManager.playbackMode == .normal ? .secondary : Color.dynamicAccent(for: musicManager.currentSong))
                        }
            ...
            VStack(spacing: 10) {
                ...
                HStack(spacing: 14) {
                    ...
                    .tint(Color.dynamicAccent(for: musicManager.currentSong))
                    .frame(maxWidth: 160)
                ...
                ForEach(Array(viewModel.songs.enumerated()), id: \.element.id) { index, song in
                    HStack(spacing: 0) {
                        ...
                        Button(action: {
                            ...
                            ZStack {
                                Circle()
                                    .fill(Color.dynamicAccent(for: musicManager.currentSong).opacity(0.15))
                                    .frame(width: 36, height: 36)
                                
                                Image(systemName: "plus")
                                    .font(.system(size: 16, weight: .semibold))
                                    .foregroundColor(Color.dynamicAccent(for: musicManager.currentSong))
                            }
                        }
                        .padding(.trailing, 20)
                        .buttonStyle(PlainButtonStyle())
                    ...
                    .background(
                        ...
                        Rectangle()
                            .fill(song.id == musicManager.currentSong?.id ? 
                                  Color.dynamicAccent(for: musicManager.currentSong).opacity(0.15) : 
                                  Color.clear)
